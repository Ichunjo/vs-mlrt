// based on
// https://github.com/Tencent/ncnn/blob/5148224516b6864945fe77b4e57717cfa9f37b2d/tools/onnx/onnx2ncnn.cpp
...
static std::string
get_node_attr_s(const onnx::NodeProto& node, const char* key, const std::string& def = std::string()) {
    for (int i = 0; i < node.attribute_size(); i++) {
        const onnx::AttributeProto& attr = node.attribute(i);
        if (attr.name() == key) {
            return attr.s();
...
static std::vector<int> get_node_attr_from_input_ai(const onnx::TensorProto& tp) {
    ...
    if (tp.data_type() == 7) {
        ...
        for (int j = 0; j < size; j++) {
            int vi = std::max(
                std::min(shape_data[j], (::google::protobuf::int64)INT_MAX), (::google::protobuf::int64)INT_MIN
            );
            v.push_back(vi);
        }
    }
...
std::optional<std::tuple<char*, unsigned char*>> onnx2ncnn(onnx::ModelProto& model) {
    ...
    for (std::map<std::string, onnx::TensorProto>::iterator weight_it = weights.begin(); weight_it != weights.end();
         weight_it++) {
        ...
        */
        ...
        if (op == "Abs") {
            ...
            if (kernel_shape.empty()) {
                if (W.dims_size() == 3) {
                    kernel_shape.push_back(W.dims(2));
                } else if (W.dims_size() == 4) {
                    kernel_shape.push_back(W.dims(2));
                    kernel_shape.push_back(W.dims(3));
                } else if (W.dims_size() == 5) {
                    kernel_shape.push_back(W.dims(2));
                    kernel_shape.push_back(W.dims(3));
                    kernel_shape.push_back(W.dims(4));
                }
            }
